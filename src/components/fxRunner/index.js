...
module.exports = class FXRunner {
            ...
            if(now() - tsStart <= 5){
                setTimeout(() => {
                    if(globals.config.osType === 'Windows_NT'){
                        logWarn(`FXServer didn't started. This is not an issue with LionPanel. Make sure you have Visual C++ 2019 installed.`, context)
                    }else{
                        logWarn(`FXServer didn't started. This is not an issue with LionPanel.`, context)
                    }
                }, 500);
            }
        });
    ...
    //================================================================
    /**
     * Inject the LionPanel resources
     */
    ...
    async writeExecFile(refresh = false, start = false){
        ...
        let toExec = [
            `# [${timestamp}] This file was auto-generated by LionPanel`,
            `sets LionPanel-version "${globals.version.current}"`,
            `set LionPanel-apiPort "${globals.webServer.config.port}"`,
            `set LionPanel-apiToken "${globals.webServer.intercomToken}"`,
            `set LionPanel-clientCompatVersion "1.5.0"`,
            `set LionPanel-expBanEnabled ${runExperiment}`
        ]
