...
//================================================================
/**
 * Reset the fxserver resources/[LionPanelClient-cache] folder.
 */
async function resetCacheFolder(basePath) {
    ...
    try {
        await fs.emptyDir(`${resFolder}/[LionPanelClient-cache]/`);
    } catch (error) {
        throw new Error(`[resetCacheFolder] Error resetting [LionPanelClient-cache] folder: ${error.message}`);
    }
...
function getResourcesList() {
    try {
        ...
        throw new Error(`[getResourcesList] Error resetting [LionPanelClient-cache] folder: ${error.message}`);
    }
}
...
async function inject(basePath, resList) {
    try {
        ...
        let rootDir = './extensions/';
        let cachePath = `${basePath}/resources/[LionPanelClient-cache]`;
        let promises = []
        resList.forEach((res) => {
