...
module.exports = async function action(res, req) {
    ...
    //==============================================
        ...
        if(parameter.length){
            ...
            cmd = `txaKickAll "LionPanel Web Panel"`;
        }
        webUtils.appendLog(req, cmd, context);
        let toResp = await globals.fxRunner.srvCmdBuffer(cmd);
        return sendAlertOutput(res, toResp);
    ...
    //==============================================
        ...
        if(!ensurePermission('commands.resources', res, req)) return false;
        webUtils.appendLog(req, 'Re-Injected LionPanel resources', context);
    ...
    //==============================================
        ...
        if(toResp.includes('Pong!')){
            return res.send({
                type: 'success',
                message: `<b>LionPanelClient is running!<br> <pre>${xss(toResp)}</pre>`
            });
        }else{
            return res.send({
                type: 'danger',
                message: `<b>LionPanelClient is not running!<br> <pre>${xss(toResp)}</pre>`
            });
        }
