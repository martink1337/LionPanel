...
function processEventTypes(event){
    ...
    }else if(event.action === 'LionPanelClient:Started'){
        return `LionPanelClient Logger started`;

    }else if(event.action === 'DebugMessage'){
        let message = event.data || 'unknown';
        return `LionPanelClient Debug Message: <span class="text-warning">${xss(message)}</span>`;
